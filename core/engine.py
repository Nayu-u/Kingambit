...
class ForensicsEngine:
    ...
    def extract_srm_noise_variance(self, gray_image: np.ndarray) -> float:
        ...
        gray_float = np.asarray(gray_image, dtype=np.float32)
        residual = cv2.filter2D(gray_float, cv2.CV_32F, srm_kernel)
        ...
        cropped = residual[:n_rows * block_size, :n_cols * block_size]
    ...
    def _analyze_forensics(self, image_bytes: bytes) -> Dict[str, Any]:
        ...
        srm_kernel = np.array([[-1, 2, -1], [2, -4, 2], [-1, 2, -1]], dtype=np.float32) / 4.0
        residual_srm = cv2.filter2D(gray.astype(np.float32), cv2.CV_32F, srm_kernel)
        block_size = 16
        h, w = residual_srm.shape
        n_rows = h // block_size
        n_cols = w // block_size
        if n_rows > 0 and n_cols > 0:
            cropped = residual_srm[:n_rows * block_size, :n_cols * block_size]
            blocks = cropped.reshape(n_rows, block_size, n_cols, block_size).transpose(0, 2, 1, 3)
            heatmap = np.var(blocks, axis=(2, 3)).astype(np.float32)
        else:
    ...
    def _interpret_metrics(
        ...
        if ela_media < 10:
            conclusions.append({
                ...
                "valor": ela_media,
                "interpretacao": "Niveis de compressao uniformes. Compativel com arquivo original de alta fidelidade sem perdas ou imagem sintetizada.",
                "severidade": "baixa"
            })
        elif ela_media <= 35:
            conclusions.append({
                "area": "ELA",
                "indicador": "Variacao ELA normal",
                "valor": ela_media,
                "interpretacao": "A imagem apresenta distribuicao de erro de compressao compativel com fotografias convencionais.",
                "severidade": "baixa"
            })
        else:
            conclusions.append({
                ...
                "valor": ela_media,
                "interpretacao": "A imagem apresenta diferencas acentuadas de compressao. Pode indicar multiplas recompreensoes ou colagem de fontes distintas.",
                "severidade": "media"
            })
        ...
        if ela_desvio < 6:
            conclusions.append({
                ...
                "valor": ela_desvio,
                "interpretacao": "Distribuicao de compressao relativamente homogenea.",
                "severidade": "baixa"
            })
        else:
            conclusions.append({
                "area": "ELA",
                "indicador": "Desvio ELA equilibrado",
                "valor": ela_desvio,
                "interpretacao": "Dispersao do erro de compressao compativel com cena fotografica natural.",
                "severidade": "baixa"
            })
            
        if variancia_ruido < 1.0:
            conclusions.append({
                "area": "Ruido",
                "indicador": "Ruido residual muito baixo",
                "valor": variancia_ruido,
                "interpretacao": "Ruido termico residual em alta frequencia extremamente atenuado. Pode indicar forte reducao de ruido digital ou geracao artificial.",
                "severidade": "baixa"
            })
        elif variancia_ruido > 90:
            conclusions.append({
                "area": "Ruido",
                "indicador": "Ruido residual elevado",
                "valor": variancia_ruido,
                "interpretacao": "Dispersao de ruido alta entre blocos. Pode indicar montagem com fontes de granulometria distinta ou condicoes de altissimo ISO.",
                "severidade": "baixa"
            })
        else:
            conclusions.append({
                ...
                "valor": variancia_ruido,
                "interpretacao": "A distribuicao estatistica de ruido termico e compativel com captura optica por sensor fisico.",
                "severidade": "baixa"
            })
            
        if fft_simetria > 0.992:
            conclusions.append({
                "area": "FFT",
                "indicador": "Simetria espectral muito elevada",
                "valor": fft_simetria,
                "interpretacao": "Espectro de frequencias apresenta regularidade muito alta. Pode indicar repeticao de grade caracteristica de geradores neurais.",
                "severidade": "baixa"
            })
        else:
            conclusions.append({
                ...
                "valor": fft_simetria,
                "interpretacao": "O espectro 2D de frequencias exibe distribuicao natural e assimetria compativel com fotografias reais.",
                "severidade": "baixa"
            })
        ...
        if mean_corr > 0.99:
            conclusions.append({
                "area": "Correlacao RGB",
                "indicador": "Correlacao intercanal muito alta",
                "valor": round(mean_corr, 4),
                "interpretacao": "Os canais de cor estao altamente correlacionados. Tipico de cenas monocromaticas, neblina ou iluminacao uniforme.",
                "severidade": "baixa"
            })
        else:
            conclusions.append({
                ...
                "valor": round(mean_corr, 4),
                "interpretacao": "A relacao entre os canais de cor e consistente com uma fotografia colorida convencional.",
                "severidade": "baixa"
            })
        ...
        if aberracao < 0.02:
            conclusions.append({
                ...
                "indicador": "Aberracao cromatica minima",
                "valor": aberracao,
                "interpretacao": "Desalinhamento cromatico quase nulo. Compativel com correcao automatica de lente (smartphones/camerasm modernas) ou renderizacao digital.",
                "severidade": "baixa"
            })
        elif aberracao > 0.22:
            conclusions.append({
                "area": "Aberracao Cromatica",
                "indicador": "Aberracao cromatica acentuada",
                "valor": aberracao,
                "interpretacao": "Dispersao cromatica pronunciada em bordas. Pode indicar lentes de grande abertura, zoom periferico ou composicao.",
                "severidade": "baixa"
            })
        else:
            conclusions.append({
                ...
                "valor": aberracao,
                "interpretacao": "O nivel de dispersao optica nas bordas esta dentro dos parametros tipicos de lentes reais.",
                "severidade": "baixa"
            })
        ...
        if grad_media < 3.5:
            conclusions.append({
                "area": "Gradiente",
                "indicador": "Transicoes suaves",
                "valor": grad_media,
                "interpretacao": "A imagem apresenta transicoes suaves. Pode indicar suavizacao digital ou baixa frequencia espacial na cena.",
                "severidade": "baixa"
            })
        else:
            conclusions.append({
                ...
                "valor": grad_media,
                "interpretacao": "O padrao de contraste e gradientes e consistente com uma fotografia natural.",
                "severidade": "baixa"
            })
        ...
        if high_severity_count >= 2:
            summary = "Multiplos indicadores apontam forte possibilidade de manipulacao ou sintese artificial."
        else:
            summary = "Os indicadores fisicos sao sinais auxiliares e nao determinam, isoladamente, a origem da imagem."
