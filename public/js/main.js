...
function selecionarArquivo(arquivo) {
    ...
    uploadTexto.textContent = 'Imagem original pronta. Selecione outro arquivo para alterar.';
    btnAnalisar.disabled = false;
    esconderErro();
    mostrarSucesso('Imagem original pronta para análise.');
...
function atualizarGauge(score, bancada) {
    ...
    if (bancada && bancada.classificacao === 'indisponivel') {
        progresso.style.stroke = 'var(--text-muted)';
        statusBadge.className = 'bancada-status-badge status-medio';
        statusBadge.textContent = 'Análise Indisponível';
    } else if (bancada && bancada.classificacao === 'suspeita_alta') {
    ...
    } else if (bancada && bancada.classificacao === 'inconclusivo') {
        progresso.style.stroke = 'var(--status-warning)';
        statusBadge.className = 'bancada-status-badge status-medio';
        statusBadge.textContent = 'Resultado Inconclusivo';
    } else if (bancada && bancada.classificacao === 'indicios') {
        progresso.style.stroke = 'var(--status-warning)';
        statusBadge.className = 'bancada-status-badge status-medio';
    ...
    btnAnalisar.addEventListener('click', function() {
        ...
        fetch('/analyze/full', { method: 'POST', body: fd, signal: controller.signal })
            ...
            .then(function(d) {
                ...
                atualizarGauge(bancada.media_geral, bancada);
                ...
                if (dadosPrincipal) {
                    ...
                    htmlPrincipal += '    <span class="jurado-principal-peso">Peso: 3x</span>';
                    htmlPrincipal += '  </div>';
                    htmlPrincipal += '</div>';
                }
                ...
                var modelosAuxiliares = [
                    ...
                    { id: 'modelo_5', padrao: 'IA Faces' },
                    { id: 'modelo_6', padrao: 'IA Cenarios' }
                ];
                ...
                var metricasDicionario = [
                    { v: forense.variancia_ruido, l: 'Variância Ruído SRM', threshold: 3, rule: 'abaixo', desc: 'Sinal auxiliar sensível a redução de ruído, compressão e características da cena.' },
                    { v: forense.fft_simetria, l: 'Simetria Fourier (FFT)', threshold: 0.95, rule: 'acima', desc: 'Sinal auxiliar de distribuição de frequências; não determina a origem da imagem.' },
                    { v: forense.aberracao_cromatica, l: 'Aberração Cromática', threshold: 0.03, rule: 'abaixo', desc: 'Sinal auxiliar afetado por correção óptica, lente, redimensionamento e processamento.' },
                    { v: forense.correlacao_rgb.rg, l: 'Correlação R-G', threshold: 0.98, rule: 'acima', desc: 'Sinal auxiliar dependente das cores e da iluminação da cena.' },
                    { v: forense.correlacao_rgb.rb, l: 'Correlação R-B', threshold: 0.98, rule: 'acima', desc: 'Sinal auxiliar dependente das cores e da iluminação da cena.' },
                    { v: forense.correlacao_rgb.gb, l: 'Correlação G-B', threshold: 0.98, rule: 'acima', desc: 'Sinal auxiliar dependente das cores e da iluminação da cena.' },
                    { v: forense.gradiente_media, l: 'Média de Gradientes', threshold: 5, rule: 'abaixo', desc: 'Sinal auxiliar influenciado por foco, textura, escala e conteúdo da cena.' },
                    { v: forense.gradiente_desvio, l: 'Desvio de Gradientes', threshold: 8, rule: 'abaixo', desc: 'Sinal auxiliar influenciado por foco, textura, escala e conteúdo da cena.' },
                    { v: forense.ela_media, l: 'Média ELA %', threshold: 15, rule: 'abaixo', desc: 'Sinal auxiliar influenciado por formato, compressão e recompressões anteriores.' }
                ];
